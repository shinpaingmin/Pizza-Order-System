                                ...
                                @if (isset($admins) && count($admins) > 0)
                                    ...
                                    <tr class="tr-shadow">
                                        ...
                                        <td>
                                            ...
                                            @if (empty($admin->image) && $admin->gender === 'male')
                                                <img src="{{ asset('image/male.png') }}" alt="Admin Profile" class="img-thumbnail shadow-sm object-cover w-100 h-100 " style="object-position: center"/>
                                            @elseif (empty($admin->image) && $admin->gender === 'female')
                                                <img src="{{ asset('image/female.png') }}" alt="Admin Profile" class="img-thumbnail shadow-sm object-cover w-100 h-100 " style="object-position: center"/>
                                            @else
                                            <img src="{{ asset('storage/' . $admin->image) }}" alt="admin profile" class="img-thumbnail shadow-sm object-cover w-100 h-100 " style="object-position: center">
                                            @endif
                                            </div>
                                        </td>
                                        <td>
                                            <span class="text-capitalize" style="width: 100px !important; display:inline-block">{{ $admin->username }}</span>
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            <span style="width: 200px !important; display:inline-block">{{ $admin->address }}</span>
                                        </td>
                                        <td>
                                        ...
                                        <td>{{ $admin->updated_at->format('F j, Y') }}</td>
                                        <td>
                                            <div class="table-data-feature justify-content-center">
                                                <button class="item mr-4" data-toggle="tooltip" data-placement="top" title="Send">
                                                    <i class="zmdi zmdi-mail-send"></i>
                                                </button>
                                                <a class="item mr-4" data-toggle="tooltip" data-placement="top" @if (Auth::user()->id !== $admin->id)
                                                    title="Demote to User" href="{{ route('admin#demote', $admin->id) }}"
                                                 @endif>
                                                    @if (Auth::user()->id !== $admin->id)
                                                    <i class="fa-solid fa-person-circle-minus"></i>
                                                    @endif
                                                </a>
                                                <a class="item mr-4 cursor-pointer" data-toggle="tooltip" data-placement="top" @if (Auth::user()->id !== $admin->id)
                                                   title="Delete" href="{{ route('admin#delete', $admin->id) }}"
                                                @endif
                                                >
                                                    @if (Auth::user()->id !== $admin->id)
